  ...
  return (
    <p className="skill">
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path
          d="M4 5.5A1.5 1.5 0 0 1 5.5 4H18a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5.5A1.5 1.5 0 0 1 4 18.5v-13ZM7 8h9M7 12h9"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.7"
          strokeLinecap="round"
      ...
      Learned skill <b>{name}</b>
    </p>
  );
}

function Summarizing({ done, data }) {
  if (!done) {
    return (
      <p className="summarize is-running">
        <span className="spin" />
  ...
  return (
    <p className="summarize">
      ...
      Summarized {data.before} messages into {data.after}
    </p>
  );
}
...
function Usage({ message }) {
  ...
  return (
    <p className="usage">
      <span>↑ {fmt(message.input)} input tokens</span>
      <span>↓ {fmt(message.output)} output tokens</span>
      {message.cached > 0 && (
        <span className="usage-cached">{fmt(message.cached)} cached</span>
      )}
    </p>
  );
}
...
function Interrupt({ message, onAnswer, answered }) {
  ...
  const submit = (event) => {
    ...
    onAnswer(questions.map((q) => ({ question: q.question, answer: values[q.id] })));
  };
  ...
  return (
    <form className={answered ? "ask ask--done" : "ask"} onSubmit={submit}>
      ...
      {!answered && <button className="ask-send" disabled={!complete}>Send</button>}
    </form>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      // the completion event replaces the running indicator in place
      if (message.type === "event" && message.name === "summarized") {
        setMessages((current) => {
          const next = [...current];
          for (let i = next.length - 1; i >= 0; i--) {
            if (next[i].type === "event" && next[i].name === "summarizing") {
              next[i] = { ...next[i], done: true, data: message.data };
              return next;
            }
          }
          return next;
  ...
  return (
    <div className={started ? "shell" : "shell shell--empty"}>
          ...
          <ol className="messages">
            {messages.map((message) => (
              <li key={message.id} className={`message message--${message.type}`}>
                ...
                {message.type === "skill" && <Skill name={message.name} />}

                {message.type === "usage" && <Usage message={message} />}

                {message.type === "event" && message.name === "summarizing" && (
                  <Summarizing done={message.done} data={message.data} />
                )}
      ...
      <form className="composer" onSubmit={send}>
        <label className="visually-hidden" htmlFor="draft">Message</label>
